...
def db_upsert_customer(name: str, phone: str, address: str, email: str | None):
    ...
    res = (
        supabase.table("customers")
                .upsert(row, on_conflict="phone")   # phone should be UNIQUE
                .execute()
    )
    if res.error:
        raise RuntimeError(res.error['message'])
    return res.data[0]          # the inserted or existing row
...
async def handle_create_customer(params: Dict[str, Any], call_id: str = None) -> str:
    ...
    try:
        ...
        if customer.get("created_at") == customer.get("updated_at"):
            # Newly created
...
async def handle_place_order(params: Dict[str, Any], call_id: str = None) -> str:
    ...
    try:
        ...
        # Make sure the customer exists (create if missing)
        customer = db_upsert_customer(
        ...
        return (f"Excellent! Your order has been placed successfully. "
                ...
                f"cylinders on {delivery_date or 'tomorrow'} for a total of "
