...
async def handle_place_order(params: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(f"Placing order: {phone}, {cylinder_size}, qty: {quantity}")
...
async def handle_get_order_status(params: Dict[str, Any]) -> str:
    ...
    try:
        ...
        delivery_date = "tomorrow"  # Default, or calculate based on created_at
...
async def root():
    ...
    return {
        ...
        "version": "1.0.0",
        "status": "active",
        "endpoints": ["/tools", "/health", "/test-db"]
    }
...
async def test_tools(tool_name: str, params: Dict[str, Any]):
    ...
    try:
        if tool_name == "create_customer":
            return {"result": await handle_create_customer(params)}
        elif tool_name == "place_order":
            return {"result": await handle_place_order(params)}
        elif tool_name == "get_order_status":
            return {"result": await handle_get_order_status(params)}
        else:
